...
class RequiredIf(click.Option):
    def __init__(self, *args, **kwargs):
        ...
        kwargs["help"] = (
            kwargs.get("help", "")
            + f" NOTE: This argument requires setting {self.required_if}"
        ).strip()
...
@click.option(
    "--endpoint-url",
    help="Overwrite the S3 endpoint URL, e.g. when using a non-AWS S3 bucket.",
)
@click.option(
    "--access-key-id",
    help="Set the access key ID used for authentication.",
    cls=RequiredIf,
    required_if="secret_access_key",
)
@click.option(
    "--secret-access-key",
    help="Set the secret access key used for authentication.",
    cls=RequiredIf,
